...
export function TerminalView({
  ...
  const [lines, setLines] = useState<Line[]>([
    {
      ...
      text:
        lang === 'zh'
          ? 'crazyczy shell 已连接。输入 help 查看命令。'
          : 'crazyczy shell connected. Type help for commands.',
    },
  ]);
  ...
  function execute(raw: string) {
    ...
    if (name === 'clear') {
      ...
      setLines((current) => [
        ...current,
        ...nextLines,
        { kind: 'output', text: 'home/  blog/  projects/  games/  about/' },
      ]);
    } else if (name === 'pwd') {
      setLines((current) => [
        ...current,
        ...nextLines,
        { kind: 'output', text: '/crazyczy/' + lang + '/terminal' },
      ]);
    } else if (name === 'whoami') {
      setLines((current) => [
        ...current,
        ...nextLines,
        { kind: 'output', text: 'Zachary Cheng / 程致远' },
      ]);
    } else if (name === 'date') {
      setLines((current) => [
        ...current,
        ...
        {
          kind: 'output',
          text: new Date().toLocaleString(lang === 'zh' ? 'zh-CN' : 'en'),
        },
      ]);
    } else if (name === 'lang') {
      ...
      if (!argument) {
        setLines((current) => [
          ...
          {
            kind: 'error',
            text: lang === 'zh' ? 'cd: 缺少目录' : 'cd: missing directory',
          },
        ]);
      } else if (
        argument === '..' ||
        ...
        setLines((current) => [
          ...
          ...nextLines,
          {
            ...
            text:
              ...
              argument +
              ': ' +
              (lang === 'zh' ? '没有这个目录' : 'no such directory'),
          },
        ]);
      }
  ...
  function historyKey(event: KeyboardEvent<HTMLInputElement>) {
    if (!['ArrowUp', 'ArrowDown'].includes(event.key) || history.length === 0)
      return;
  ...
  return (
    <section
      ...
      aria-label={lang === 'zh' ? '交互式终端' : 'Interactive terminal'}
    >
      ...
      <div className="terminal-output" aria-live="polite">
        {lines.map((line, index) => (
          <p key={index} className={'terminal-line ' + line.kind}>
            {line.kind === 'command' && (
              <span className="terminal-prompt">guest@crazyczy:~$ </span>
            )}
            {line.text}
          </p>
        ))}
        ...
        >
          <label
            htmlFor={onNavigate ? 'room-terminal-input' : 'terminal-input'}
            className="terminal-prompt"
          >
            guest@crazyczy:~$
          </label>
          <input
            id={onNavigate ? 'room-terminal-input' : 'terminal-input'}
            value={value}
            ...
            autoCapitalize="none"
            spellCheck={false}
            aria-label={lang === 'zh' ? '输入命令' : 'Enter command'}
          />
        </form>
      </div>
    </section>
