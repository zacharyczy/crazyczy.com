    ...
    <SiteShell lang={lang} active="games">
      <section className="page-wrap">
        ...
        <p className="page-lead">
          {lang === 'zh'
            ? '两段可以随时开始的小型像素游戏。分数只保存在你的设备上。'
            : 'Two small pixel games ready whenever you are. Scores stay on your device.'}
        </p>
        <div className="game-grid">
          {games.map((game) => (
            <a
              key={game.slug}
              href={pageHref(lang, `games/${game.slug}`)}
              className="game-card"
            >
              <div className="game-card-meta">
                <span>{game.number}</span>
                <GameBest storageKey={game.storageKey} lang={lang} />
              </div>
