...
export function AboutView({ lang }: { lang: Language }) {
  ...
  return <SiteShell lang={lang} active="about"><section className="page-wrap"><div className="grid gap-12 lg:grid-cols-[.65fr_1.35fr]"><div><p className="eyebrow">Profile / CZY</p><h1 className="page-title">{t.aboutTitle}</h1></div><div className="pt-3"><p className="text-2xl leading-10 tracking-[-.035em] text-slate-200">{t.aboutLead}</p><div className="mt-10 space-y-6 text-base leading-8 text-slate-500">{paragraphs.map((p) => <p key={p}>{p}</p>)}</div><dl className="mt-12 grid grid-cols-2 gap-6 border-t border-white/8 pt-8 font-mono text-xs"><div><dt className="text-slate-600">Location</dt><dd className="mt-2 text-slate-300">Shanghai · UTC+8</dd></div><div><dt className="text-slate-600">Focus</dt><dd className="mt-2 text-slate-300">Web · Product · Writing</dd></div></dl></div></div></section></SiteShell>;
}
