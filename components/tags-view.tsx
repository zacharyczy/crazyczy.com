  ...
  return (
    <SiteShell lang={lang} path="tags">
      <section className="page-wrap">
        <p className="eyebrow">
          Index / {tags.length.toString().padStart(2, '0')}
        </p>
        <h1 className="page-title">{copy[lang].tags}</h1>
        <div className="mt-14 grid gap-3 sm:grid-cols-2">
          {tags.map((tag) => (
            <section
              key={tag}
              className="rounded-xl border border-white/8 bg-white/[.02] p-5"
            >
              <div className="flex items-center justify-between">
                <h2 className="font-mono text-sm text-cyan-200">#{tag}</h2>
              ...
              {getPosts(lang)
                ...
                .map((post) => (
                  ...
                  >
                    {post.title} <span className="text-slate-700">↗</span>
                  </a>
                ))}
            </section>
          ))}
