...
const TEXT = {
  en: {
    ...
    empty: [
      'No articles yet.',
      'Personal essays are gathering here.',
      'The poems are still drying by the window.',
    ],
    read: 'Read',
  },
  zh: {
...
function category(post: HomePost) {
  ...
  if (tags.some((tag) => tag === 'poem' || tag === 'poetry' || tag === '诗歌'))
    return 2;
  if (tags.some((tag) => tag === 'essay' || tag === 'notes' || tag === '随笔'))
    return 1;
  return 0;
}
...
export function WritingIndex({
  ...
  return (
    <section
      className={archive ? 'writing-showcase archive' : 'writing-showcase'}
      id="writing"
    >
      <div className="writing-heading">
        <div>
          <p className="eyebrow">Writing / 0{active + 1}</p>
          <h1>{t.title}</h1>
        </div>
        <p>{t.hint}</p>
      </div>
        ...
        <div className="writing-list">
          {visible.map((post, index) => (
            ...
            >
              ...
              <div>
                <time>{post.publishDate.replaceAll('-', '.')}</time>
                <h2>{post.title}</h2>
