  ...
  return (
    ...
    >
      <time className="font-mono text-[11px] tracking-wider text-slate-600">
        {post.publishDate.replaceAll('-', '.')}
      </time>
      <div>
        <h3 className="text-base font-medium text-slate-200 transition group-hover:text-cyan-200">
          {post.title}
        </h3>
        ...
        <div className="mt-3 flex gap-2">
          {post.tags.map((tag) => (
            <span key={tag} className="font-mono text-[10px] text-slate-600">
              #{tag}
            </span>
          ))}
        </div>
      </div>
      <span className="mt-1 hidden text-slate-700 transition group-hover:text-cyan-300 sm:block">
        ↗
      </span>
    </a>
  );
